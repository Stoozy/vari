use crate::{token::{Token, TokenType}, expr::Expr, vari::VARI};
...
impl Parser {
    ...
    fn primary(&mut self) -> Expr {
        if self.match_list(vec![TokenType::FALSE])  {
            return Expr::Literal { value: Box::new(VariTypes::Boolean(false)) };
        }
        ...
        if self.match_list(vec![TokenType::NIL]) {
            return Expr::Literal { value: Box::new(VariTypes::Nil) };
        }

        if self.match_list(vec![TokenType::NUMBER, TokenType::STRING]) {
            return Expr::Literal { value: self.prev_token().literal.unwrap() }
        }
    ...
    fn unary(&mut self) -> Expr {
        if self.match_list(vec![TokenType::NOT, TokenType::MINUS]) {
            ...
            let rhs = self.unary();
            return Expr::Unary { op: operator, rhs: Box::new(rhs) };
        }
    ...
    fn factor(&mut self) -> Expr {
        ...
        while self.match_list(vec![TokenType::SLASH, TokenType::STAR]) {
            ...
            expr = Expr::Binary { lhs: Box::new(expr), op: operator, rhs: Box::new(rhs) };
        }
    ...
    fn comparison(&mut self) -> Expr {
        ...
        while self.match_list(vec![TokenType::GT, TokenType::GE, TokenType::LT, TokenType::LE]) {
            let operator : Token = self.prev_token();
